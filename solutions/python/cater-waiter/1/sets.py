...
def check_drinks(drink_name, drink_ingredients):
    ...
    """
    ing_s = set(drink_ingredients)
    if ing_s.isdisjoint(ALCOHOLS):
...
def tag_special_ingredients(dish):
    ...
    """
    itsc = set(dish[1])
    special = itsc.intersection(SPECIAL_INGREDIENTS)
...
def separate_appetizers(dishes, appetizers):
    ...
    """
    dish_only = set(dishes).difference(set(appetizers))
    return list(dish_only)
...
def singleton_ingredients(dishes, intersection):
    ...
    for i in dishes:
        all_dishes.update(i)
    single_ing = all_dishes - intersection
    return single_ing
